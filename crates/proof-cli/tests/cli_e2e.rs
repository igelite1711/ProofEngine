...
/// M1 remediation (BUG): wrapper `kind` is authoritative for proof envelopes.
/// When present it MUST be exactly "proof"; any other string is a kind
/// mismatch → FAIL verdict Ok(1) with SCHEMA_VIOLATION JSON (never VALID).
/// Missing `kind` is allowed for raw transports (bare cbor+id); non-string
/// `kind` is a malformed wrapper → Err (exit 2). Covers: proof/event/evil/
/// unknown/missing/malformed. Single interpretation: PROOF-ENGINE-SPEC §15.
#[test]
fn proof_wrapper_kind_enforced() {
    let w = tmpdir("wrapper-kind");
    let (ev1, att) = (format!("{w}/ev1.json"), format!("{w}/att.json"));
    let proof = format!("{w}/proof.json");
    run(args(
        "create-event",
    ...
    // 1. kind == "proof" → PASS.
    assert_eq!(run(base_args(&proof)), Ok(0), "kind proof must verify");
    // Library-level: kind-mismatch report is Invalid + SCHEMA_VIOLATION.
    let rep = proof_cli::check::envelope_kind_mismatch_report("event");
    assert_eq!(rep.cryptographic_validity, proof_verify::Validity::Invalid);
    assert!(rep
        .failure_codes()
        .contains(&proof_core::ErrorCode::SchemaViolation));
    // 2/3/4. event / evil / unknown → FAIL Ok(1), never VALID.
    for bad_kind in ["event", "evil", "unknown"] {
        let mut v: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(&proof).unwrap()).unwrap();
        v["kind"] = serde_json::Value::String(bad_kind.into());
        let bad = format!("{w}/proof-kind-{bad_kind}.json");
        std::fs::write(&bad, serde_json::to_string_pretty(&v).unwrap()).unwrap();
        assert_eq!(
            run(base_args(&bad)),
            Ok(1),
            "kind {bad_kind} must be FAIL Ok(1)"
        );
    }
    // 5. missing kind → raw transport, still PASS.
    let mut v: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&proof).unwrap()).unwrap();
    v.as_object_mut().unwrap().remove("kind");
    let nokind = format!("{w}/proof-no-kind.json");
    std::fs::write(&nokind, serde_json::to_string_pretty(&v).unwrap()).unwrap();
    assert_eq!(
        run(base_args(&nokind)),
        Ok(0),
        "missing kind must still load as raw transport"
    );
    // 6. malformed kind (non-string) → transport error Err (exit 2 path).
    let mut v: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&proof).unwrap()).unwrap();
    v["kind"] = serde_json::Value::Number(42.into());
    let malformed = format!("{w}/proof-kind-malformed.json");
    std::fs::write(&malformed, serde_json::to_string_pretty(&v).unwrap()).unwrap();
    let err = run(base_args(&malformed)).unwrap_err();
    assert!(
        err.contains("wrapper `kind` must be a string"),
        "malformed kind must name the problem, got: {err}"
    );
}
...
/// M5 remediation (SECURITY HARDENING): --production rejects raw secrets in
/// argv (--seed/--value), requiring --seed-file/--value-file. Non-production
/// retains --seed ergonomics with a warning. Shell history + ps risks in help.
#[test]
fn production_rejects_argv_secrets() {
    let w = tmpdir("prod-secrets");
    let sf = format!("{w}/seed.hex");
    let seed_hex = "0a".to_string() + &"0b".repeat(31);
    assert_eq!(seed_hex.len(), 64);
    std::fs::write(&sf, format!("{seed_hex}\n")).unwrap();
    // attest --seed in --production → Err requiring --seed-file.
    let err = run(args(
        "attest",
        &[
            "--seed",
    ...
    .unwrap_err();
    assert!(
        err.contains("--seed-file"),
        "production --seed must require --seed-file, got: {err}"
    );
    // attest --seed-file in --production → OK (ergonomics preserved).
    run(args(
        "attest",
        &[
            "--seed-file",
    ...
    .unwrap();
    // commit --value in --production → Err requiring --value-file.
    let cerr = run(args(
        "commit",
        &["--salt", "s", "--value", "secret", "--production"],
    ))
    .unwrap_err();
    assert!(
        cerr.contains("--value-file"),
        "production --value must require --value-file, got: {cerr}"
    );
}
...
/// External-review regression (M4): omitting --revocations-known-at fails
/// closed with lifecycle UNKNOWN (exit 1), never VALID. Fail-closed preserved.
#[test]
fn missing_freshness_fails_closed_unknown() {
    let w = tmpdir("freshness-omitted");
    let (ev1, att) = (format!("{w}/ev1.json"), format!("{w}/att.json"));
    let proof = format!("{w}/proof.json");
    run(args(
        "create-event",
    ...
    .unwrap();
    // No --revocations-known-at and no --no-require-status: UNKNOWN → exit 1.
    assert_eq!(
        run(args(
            "verify",
            &["--proof", &proof, "--clock", "1700000300"]
        )),
        Ok(1),
        "omitted freshness must fail closed, never VALID"
    );
}

/// External-review regression (lifecycle): unauthorized status at CLI level
/// stays ACTIVE with status_inputs_valid:false (feed hygiene, not proof
/// failure). Mirrors proof-verify unauthorized_revocation_fails.
#[test]
fn unauthorized_status_stays_active_at_cli() {
    let w = tmpdir("unauth-cli");
    let (ev1, att) = (format!("{w}/ev1.json"), format!("{w}/att.json"));
    let proof = format!("{w}/proof.json");
    run(args(
        "create-event",
    ...
    .unwrap();
    let target = id_of(&proof);
    // Revoke with an unrelated key (no authority over the test-key attestation).
    let other_hex = "07".to_string() + &"07".repeat(31);
    let target_att = id_of(&att);
    let bad_status = format!("{w}/revoke-unauth.json");
    run(args(
        "revoke",
        &[
            "--seed",
    ...
    .unwrap();
    let _ = target;
    // Verify with the unauthorized feed presented but test-key as authority:
    // proof stays valid (ACTIVE); feed hygiene surfaces via STATUS stage.
    // Full JSON assertion lives at library level; CLI asserts no silent PASS downgrade.
    let out = format!("{w}/report.json");
    let rc = run(args(
        "verify",
        &[
            "--proof",
    ...
    assert_eq!(
        rc,
        Ok(0),
        "unauthorized feed must not flip a good proof: {rc:?}"
    );
    let rep: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&out).unwrap()).unwrap();
    assert_eq!(rep["status_inputs_valid"], false);
    assert!(rep["checks"]
        .as_array()
        .unwrap()
        .iter()
        .any(|c| c["code"] == "UNAUTHORIZED_STATUS"));
}

/// External-review regression (H2, INTENTIONALLY RETAINED): evidence is
/// digest-only by design. Swapping one well-formed digest for another yields
/// a *different* VALID proof id — the engine commits to digests, it does not
/// fetch/check bytes. Documents storage-neutrality; callers needing value↔
/// bytes binding use opt-in evidence_digest + content store (OPERATIONS §5).
#[test]
fn evidence_substitution_yields_different_valid_proof() {
    let w = tmpdir("evd-subst");
    let (ev1, att) = (format!("{w}/ev1.json"), format!("{w}/att.json"));
    run(args(
        "create-event",
        &[
    ...
    let d1 = "11".repeat(32);
    let d2 = "22".repeat(32);
    for (tag, digest) in [("a", d1.as_str()), ("b", d2.as_str())] {
        run(args(
            "add-evidence",
            &[
                "--kind",
    ...
    for tag in ["a", "b"] {
        assert_eq!(
            ...
            Ok(0),
            "each well-formed digest builds a valid proof"
        );
    }
    assert_ne!(
        id_of(&format!("{w}/proof-a.json")),
        id_of(&format!("{w}/proof-b.json")),
        "substitution must change proof_id (distinct commitments)"
    );
}
