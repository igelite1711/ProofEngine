...
const GLOBAL_FLAGS: &[&str] = &["help", "quiet", "json", "out", "verbose"];
...
const COMMAND_FLAGS: &[(&str, &[&str])] = &[
    ...
    (
        ...
        &[
            ...
            "expires-at",
            "evidence-ref",
            // M5: strict marker so production key hygiene can be enforced
            // without changing attestation semantics.
            "production",
        ],
    ),
    (
    ...
    (
        "revoke",
        &["seed", "seed-file", "target", "reason", "at", "production"],
    ),
    (
        "supersede",
        &[
    ...
    (
        "commit",
        &["value", "value-file", "salt", "alg", "production"],
    ),
...
impl Cli {
    ...
    /// `--verbose`: Layer-2 human output (translated stages, identifiers).
    /// Machine output (`--json`) is never routed through presentation, so
    /// verbose cannot perturb the machine contract.
...
pub fn load_key_opt(cli: &Cli) -> Result<proof_crypto::Ed25519Key, String> {
    match (cli.opt("seed"), cli.opt("seed-file")) {
        ...
        (Some(seed), None) => {
            // M5 remediation (SECURITY HARDENING): production mode must not
            // accept raw key material in argv (ps/history). Require
            // --seed-file; retain --seed for local experimentation only.
            // Shell history + process-list risks documented in help + runbook.
            if cli.has("production") {
                return Err(
                    "--seed exposes raw key material in argv (ps/history); in --production use --seed-file <path> (or stdin file) instead"
                        .into(),
                );
            }
...
USAGE
  ...
  commitment (see CONFIDENTIALITY.md). `--value` is visible to ps(1)/history
  like `--seed`; prefer `--value-file` for real secrets (--production rejects
  --value and requires --value-file). Salt must be unique per commitment.",
        "graph" => "\
...
USAGE
  ...
  Prefer --seed-file <path> over --seed <hex> (argv is visible to ps(1) and
  shell history; --production rejects --seed and requires --seed-file).
...
/// Yellow text when color is enabled, plain otherwise. Yellow is warning /
/// indeterminate decoration only: meaning always travels in the ✓/⚠/✗ mark.
pub fn yellow(s: &str) -> String {
    paint("33", s)
}

/// Marks a human-facing line with ✓/⚠/✗ plus a label. The mark is the
/// meaning (accessibility: never color alone); color is decoration.
pub fn color_mark(mark: &str, label: &str) -> String {
