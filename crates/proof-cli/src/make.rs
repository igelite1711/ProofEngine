...
pub fn verify(cli: &Cli) -> Result<i32, String> {
    ...
    let proof = match crate::check::load_proof(&proof_path, cli.quiet()) {
        ...
        Err(e) => {
            ...
            if e.contains("envelope id mismatch") {
                ...
                if !cli.quiet() {
                    crate::ux::emit_summary(&report, false, cli.verbose());
                }
                return Ok(crate::EXIT_FAIL);
            }
            // M1 (audit remediation, BUG): wrapper kind mismatch is tamper,
            // not transport. Emit a synthetic SCHEMA FAIL report (exit 1)
            // with JSON, never silently verify under a different kind.
            if e.contains("envelope kind mismatch") {
                let claimed = e
                    .split("wrapper says kind \"")
                    .nth(1)
                    .and_then(|s| s.split('"').next())
                    .unwrap_or("?");
                let report = crate::check::envelope_kind_mismatch_report(claimed);
                crate::check::emit_report(&report, cli.opt("out"), cli.quiet())?;
                if !cli.quiet() {
                    crate::ux::emit_summary(&report, false, cli.verbose());
                }
            ...
            // unknown wrapper fields, non-object wrappers, malformed kind,
            // missing files, bad JSON/hex are usage/engine errors (exit 2).
            if e.contains("wrapper has unknown field")
                || e.contains("wrapper must be a JSON object")
                || e.contains("wrapper `kind` must be a string")
            {
                return Err(e);
            }
    ...
    // M4 remediation (UX/DX, fail-closed preserved): lifecycle UNKNOWN with
    // no --revocations-known-at is the #1 first-run surprise. Pipeline is
    // correct to fail closed; teach both intents on stderr (stdout stays pure).
    // Genesis escape hatch is bootstrap/testing only, never production.
    ...
    {
        eprintln!(
            "Verification cannot establish current status (REVOCATION_UNKNOWN, fail closed)."
        );
        eprintln!(
            "Reason: revocation/status information is missing or stale (--revocations-known-at omitted)."
        );
        eprintln!(
            "For production verification: provide a current status source (--status <file> + --authority <keyref> + --revocations-known-at {} within --skew of --clock).",
            c.verified_at
        );
        eprintln!(
            "For historical verification only: re-run with explicit --revocations-known-at plus --no-require-status to assert absence (genesis/bootstrap/testing mechanism — NEVER copy into production)."
        );
    }
    crate::check::emit_report(&report, cli.opt("out"), cli.quiet())?;
    ...
    if !cli.quiet() {
        crate::ux::emit_summary_claimed(
            &report,
            Some(&proof.proof.proposition),
            strict_fail,
            cli.verbose(),
        );
        ...
        // M2 remediation (UX/DX, semantics preserved): DERIVATION vs REFERENCE.
        // Derivation cycles always fail (CYCLE_DETECTED → evidence invalid).
        // REFERENCES/EQUIVALENT cycles are linkage-valid by design
        // (citations/see-also, identity assertions) and affect linkage
        // validity only, never provenance/derivation validity. Full-DAG
        // assurance needs --production/--require-acyclic.
        ...
        let has_reference = proof
            ...
            .relationships
            .iter()
            .any(|r| matches!(r.rel_type.as_str(), "REFERENCES" | "EQUIVALENT"));
        if has_derivation && !vp.require_acyclic_provenance {
            eprintln!(
                "  note: derivation edges present; derivation cycles fail by default (CYCLE_DETECTED → provenance invalid). REFERENCES/EQUIVALENT cycles (if any) remain linkage-valid and do not affect provenance validity; use --production/--require-acyclic for full-DAG linkage assurance."
            );
        } else if has_reference && !vp.require_acyclic_provenance {
            eprintln!(
                "  note: REFERENCES/EQUIVALENT linkage present; cycles here affect linkage validity only, never provenance/derivation validity (derivation stays acyclic-enforced). Use --production/--require-acyclic to additionally reject REFERENCES cycles."
            );
        }
    }
    ...
    if explain {
        if json_mode {
            ...
            if !cli.quiet() {
                let strict_fail = (cli.has("strict-current") || cli.has("production"))
                    && !crate::check::is_currently_acceptable(&report);
                crate::ux::emit_summary(
                    &crate::ux::with_decision(&report, outcome.decision),
                    strict_fail,
                    cli.verbose(),
                );
            }
        ...
        // UX: after the machine-perc prose, close with the same two-axis
        // human summary as verify (PROOF INTEGRITY vs CLAIM DECISION), so
        // `evaluate`/`explain` end with the answer in words + next step.
        // The decision axis uses the REAL policy outcome here — the pipeline
        // report's policy_decision is always indeterminate by design, but
        // `evaluate` did evaluate, so the human line must say ACCEPTED or
        // REJECTED, never NOT EVALUATED.
        if !cli.quiet() {
            let strict_fail = (cli.has("strict-current") || cli.has("production"))
                && !crate::check::is_currently_acceptable(&report);
            let real_report = crate::ux::with_decision(&report, outcome.decision);
            crate::ux::emit_summary_claimed(
                &real_report,
                Some(&proof.proof.proposition),
                strict_fail,
