...
Precedence per attestation: `REVOKED > SUPERSEDED > EXPIRED > UNKNOWN >
ACTIVE`. Status objects targeting ids outside the proof are still
...
- Applies only if: well-formed claim AND (signer == target's original issuer
  OR signer ∈ `revocation_authorities`) AND not future-dated. Else
