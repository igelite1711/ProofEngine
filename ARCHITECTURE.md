...
├── tests/                  # end-to-end + negative + golden vectors
...
- `event_type`: open string type in V1 with well-known labels, e.g. `payment.created|payment.completed|invoice.issued|document.signed|package.shipped|sensor.measurement.recorded|ai.action.executed`. Unknown types → reject (fail closed), not warn.
...
- Revocation/supersession are separate signed objects referencing `attestation_id`, not deletions.
...
`OWNS | CREATED | SETTLES | REFERENCES | CONTAINS | PRODUCED | EXECUTED | ISSUED | SUPERSEDES | REVOKES`

```
Relationship { rel_id, from (ObjectRef), type, to (ObjectRef), evidence_ref?, attestation_ref? }
```

Edges that establish trust-relevant facts (e.g. `SETTLES`) MUST carry `evidence_ref` or `attestation_ref`; bare edges are rejected at graph-validation stage for those types (configurable per policy, default strict).
...
- Immutable once built: `proof_id = prf:v1:<b64uNoPad(sha256(canonical({proof_format_version, proposition, sorted member id sets})))>` — single definition in FORMAT.md §3 (member-set binding).
- Portable: verifier needs only Proof bytes + trust inputs (trusted issuer list, verifier clock, revocation set, policy). No DB access.
- Limits enforced on parse: `max_proof_size (default 1 MiB)`, `max_evidence_items (64)`, `max_nodes (128)`, `max_edges (256)`, `max_depth (16)`, etc.
...
INPUT → PARSE → SCHEMA → CANONICAL → IDENTIFIERS → SIGNATURES → KEYS
 → TIME/EXPIRY → REVOCATION/SUPERSESSION → EVIDENCE → RELATIONSHIPS
 → GRAPH-CONSISTENCY → POLICY → FINAL
```
...
8. REVOCATION/SUPERSESSION: join against verifier-supplied revocation set + embedded signed revocations; states `ACTIVE|REVOKED|EXPIRED|SUPERSEDED|UNKNOWN`. Missing info → `UNKNOWN` → fail closed.
...
12. POLICY: evaluate validated policy against verified state.
12. POLICY (caller-evaluated, reported INDETERMINATE by the pipeline) → 13. FINAL: emit triple `{ cryptographic_validity, evidence_validity, policy_decision }` + `explanation[]` + stable error codes.
...
`VerifyCtx { verified_at, clock_skew_leeway, trusted_issuers, allow_remote: false, allowed_algs, limits, status_objects, revocation_authorities, revocations_known_at }`.
