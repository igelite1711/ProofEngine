...
## Custom AAD (ADVANCED, isolated — remediation §14)

`sign_*_with_aad` / `verify_sign1_with_aad` exist for out-of-band adapter
domains ONLY (F11). The V1 protocol pipeline NEVER uses them: it always
verifies under `external_aad = h'504531'` ("PE1"), so a signature minted
under any other AAD fails closed (`SIGNATURE_INVALID`) and never
cross-accepts into V1 (gated by `custom_aad_never_cross_verifies_with_pe1`).
Do NOT use custom AAD for V1 proofs; agree AAD out-of-band per deployment,
never accept attacker-chosen AAD (caller-supplied AAD bounded to 1 KiB).
Invariant: protocol verification uses PE1 domain separation, always.
