...
| `176f036` (this branch, decision PENDING) | CORE (conformance hardening) | Implementation brought under frozen fail-closed invariants, 18 items (`docs/ARCHITECTURE-CHANGE-PROPOSAL-conformance-hardening.md` §3): zero-clock `ProofFresh` FAIL, revocation bound, typed refs, version-downgrade preservation, KEYS attribution, bound checks; full `VerifyCtx` on CLI (additive flags) | Confined to inputs the frozen spec already declares must fail; goldens untouched (31/31), differentials green (37+5). Merge requires the §5 maintainer decision recorded in the proposal §11 |
