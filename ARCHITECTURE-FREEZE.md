...
| F2 | Identifiers | `<prefix>:v1:<b64uNoPad(sha256(canonical))>`; full 32-byte digests only; pad-bit spellings rejected; `proof_id` binds proposition + `created_at` + exact sorted member id sets (+`referenced_proofs`/`vocabularies` keys only when non-empty; `created_at` covered — V1 CORE freeze deviation, pre-V1.0 wire fix) |
...
| F4 | Verification order | 11 check stages (`PARSE→…→GRAPH`) + `STATUS` feed-hygiene stage (V1.1 F2 fix; excluded from validity) + caller POLICY hand-off + FINAL; failures recorded never repaired; unknown never becomes valid |
| F5 | Verdict shape | Triple `{cryptographic_validity, evidence_validity, policy_decision}` + `status_inputs_valid` feed health + lifecycle/evidence-status/conflict records + additive 8-dimension projection; pipeline never emits `PASS` alone |
...
| `proof-adapter-scitt` (P7, crate + workspace member) | ADAPTER | IETF SCITT-style signed statements ↔ PE: inbound (verified statement → `scitt.statement` attestation + registration/receipt evidence + `transparency.checkpoint` content), outbound byte-identical projection; keyref-only Ed25519, X.509/DID fail closed (`SCHEMA_VIOLATION`); fixture differential pinned both directions | None — pure adapter above the frozen core; uses the open statement space and existing transparency evidence kinds; no frozen-path file touched |
| `176f036` (PR #1, approved 2026-09-12) | CORE (conformance hardening) | Implementation brought under frozen fail-closed invariants, 18 items (`docs/ARCHITECTURE-CHANGE-PROPOSAL-conformance-hardening.md` §3): zero-clock `ProofFresh` FAIL, revocation bound, typed refs, version-downgrade preservation, KEYS attribution, bound checks; full `VerifyCtx` on CLI (additive flags) | Confined to inputs the frozen spec already declares must fail; goldens untouched (31/31), differentials green (37+5). Maintainer decision recorded in the proposal §11; pin unchanged |
| `<this-commit>` (pre-V1.0 wire fix, 2026-09-12) | CORE (deviation, proposal `docs/ARCHITECTURE-CHANGE-PROPOSAL-created_at-binding.md`, §11 PENDING) | `proof_id` binding gains `"created_at"` (F2/F3 re-worded in §2): the external review's freshness gap — `proof_fresh` readable via holder re-stamp — is a foundation-level binding deficiency, unfixable by extension/adapters/profiles (the id itself must cover the timestamp) | Breaking by design: every existing `proof_id` changes; goldens regenerated (17 of 31), differentials updated in lockstep (37/37 py, 33/33 ts), restamp-attack test flipped to fail-closed, soak `created_at` exception removed. Merge requires maintainer §11 verdict; pin re-evaluated then |
| `<this-commit>` (pre-V1.0 verdict fix, 2026-09-13) | CORE (deviation, external review F2) | Status-input hygiene moves from `REVOCATION` to new `STATUS` stage: malformed/unauthorized/future-dated/supplied-signature failures no longer flip `evidence_validity`; `VerifyReport::status_inputs_valid` + `VerifiedState::status_inputs_valid` + CLI `status_inputs_valid` JSON carry feed health; lifecycle outcomes (REVOKED/COMPROMISED/UNKNOWN) unchanged in `REVOCATION` | Verdict change by design: ACTIVE proofs with bad feeds go from `evidence invalid` (+INDETERMINATE policy) to `evidence valid` (+evaluated policy) with `status_inputs_valid:false`; `failure_codes` still contains the feed error; lifecycle tests updated; pin re-evaluated then |
| `<this-commit>` (pre-V1.0 additive, 2026-09-13) | GENERIC EXTENSION | Provenance DAG profile (`check_acyclic_provenance` + `VerifyCtx::require_acyclic_provenance` + `--require-acyclic`; default off, V1 linkage semantics preserved) | None unless opted in: default proofs byte-identical verdicts; opt-in cycles fail `CYCLE_DETECTED` at GRAPH |
| `<this-commit>` (pre-V1.0 additive, 2026-09-13) | APPLICATION (CLI) | `relate` EQUIVALENT free-identity-refs parity (shaped fail-closed, grounding still required) + `inspect` single-artifact view + batch `status_inputs_valid` + API `status_inputs_valid` + runbook/confidentiality docs | None on frozen semantics: previously rejected CLI inputs now writable but verify under pre-existing graph rules; inspect read-only; docs only |
