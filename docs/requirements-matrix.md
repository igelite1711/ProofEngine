...
| PE-EVID-002 | Attestation evidence_ref must resolve in-proof (digest-only externals); opt-in claim `evidence_digest` must match bound digest (text hex, fail on non-text/missing/mismatch) | FORMAT §4.6 | `proof-verify/src/pipeline.rs` EVIDENCE stage | `proof::evidence_digest_binding_holds_when_matching` | `proof::evidence_digest_mismatch_fails_closed`, `proof::dangling_evidence_ref_fails_evidence` (added C3) |
...
| PE-GRAPH-003 | SUPERSEDES acyclic + linear + depth-bounded; derivation subgraph always acyclic (REFERENCES/EQUIVALENT linkage excluded); opt-in full-DAG profile (`require_acyclic_provenance`) additionally rejects REFERENCES cycles | FORMAT §4.4 | `proof-graph/src/lib.rs` Kahn check + `check_derivation_acyclic` + `check_acyclic_provenance` | `graph::non_supersedes_cycles_allowed`, `graph::derivation_cycles_fail_closed_by_default`, `graph::provenance_dag_allows_chains_rejects_cycles` | `graph::supersedes_cycle_rejected`, golden-10 |
...
| PE-VERIFY-008 | Stage 8 REVOCATION (lifecycle) + STATUS (feed hygiene, V1.1): signed status only + authority + strict knowledge cutoff (`issued_at<=verified_at`, no skew grace for future); empty feed ACTIVE by default with honesty note, fail-closed under `require_status_feed`/`--production`; hygiene never flips validity (`status_inputs_valid`) | ARCH §4 | `proof-verify/src/pipeline.rs` | `lifecycle::revoked_attestation_fails_with_revoked`, `lifecycle::require_status_feed_fails_closed_on_empty_feed` | `lifecycle::unauthorized_revocation_fails`, `lifecycle::future_status_within_skew_does_not_apply_to_history` |
| PE-VERIFY-009 | Stages 9–11 EVIDENCE/RELATIONSHIPS/GRAPH: derivation always acyclic (REFERENCES/EQUIVALENT excluded), full-DAG opt-in for REFERENCES | ARCH §4 | `proof-verify/src/pipeline.rs`, `proof-graph` | `proof::valid_proof_verifies_portable`, `proof::derivation_cycle_fails_by_default` | `proof::ungrounded_proof_separates_crypto_from_evidence` |
...
| PE-CLI-004 | Machine-readable reports to stdout/file; progress to stderr; verify JSON carries `currently_acceptable`, evaluate prose is decision-only (`--json`/`--out` for report) | README | `proof-cli/src/check.rs` emit_report/report_json | `cli_e2e::demo_is_deterministic_and_green`, `cli_e2e::production_profile_enforces_currency` | — |
...
| PE-CLI-006 | prose is the default and never mixes JSON over prose; `--json` gives one merged machine document for evaluate (`{policy_outcome,report}`) and explain (+ `explanation`); `--strict-current`/`--production` overlays currency on policy | ARCH §7 | `proof-cli/src/make.rs` evaluate | `policy::explanations_never_contradict_verdicts`, `cli_e2e::explain_json_carries_merged_outcome_and_explanation`, `cli_e2e::production_overlays_currency_on_evaluate_and_batch` | — |
| PE-CLI-007 | evaluate --json: machine-readable policy_outcome+report, exit contract unchanged, explain prose-only; API mirrors CLI wire strings + currency fields | POLICY.md Machine-readable output | `proof-cli/src/check.rs` outcome_json, `make.rs` evaluate, `proof-api/src/api.rs` | `policy::policy_outcome_json_matches_evaluator` | `cli_e2e::evaluate_json_flag_keeps_exit_contract` |
