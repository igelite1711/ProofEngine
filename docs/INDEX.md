# Documentation Index (H4 remediation: single authority, classified surface)

One map for all documents. **Authority rule (exactly one per layer):**
on protocol semantics `PROOF-ENGINE-SPEC.md` wins; on wire bytes
`docs/format.cddl` wins. No other document is authoritative on those layers.

Classification: NORMATIVE CORE = protocol/wire authority; REFERENCE =
as-built guide; DEVELOPER CONVENIENCE = how-to; PERIMETER/EXPERIMENTAL =
0.1.x outside V1 promise; HISTORICAL = kept, explicitly marked, never extended;
DEPRECATED = do not use. `SPEC.md` is HISTORICAL full-text draft (kept, not extended).
...
| [Protocol Spec](../PROOF-ENGINE-SPEC.md) | **NORMATIVE CORE — Protocol authority** | Unified normative protocol |
| [Full-text draft](../SPEC.md) | HISTORICAL (superseded where it differs) | Historical V1.0 text — do not extend |
...
|----------|-----------|----------------|
| [Format](../FORMAT.md) | REFERENCE (wire-format guide; CDDL wins) | How proofs are stored (file format) |
| [Operations](OPERATIONS.md) | NORMATIVE ADAPTER — ops contract | Trust registries, rotation, feeds, replay, clocks (H3) |
| [CLI](CLI.md) | REFERENCE IMPLEMENTATION — CLI guide | Primary path + 36-verb classification (H4 §9) |
