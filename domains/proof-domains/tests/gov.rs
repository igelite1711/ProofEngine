//! Government domain journey (permit vertical). Domain DATA only.
...
pub fn journey() -> BuiltProof {
    ...
    let e1 = create_event(
        proof_core::model::EventContent {
            v: 1,
            event_type: EventType::new("gov.permit.applied"),
            subject: "permit:g12".into(),
            effective_at: 1_700_000_000,
            payload_ref: HashRef::new(HashAlgorithm::Sha256, vec![0x81u8; 32]).unwrap(),
    ...
    let e2 = create_event(
        proof_core::model::EventContent {
            v: 1,
            event_type: EventType::new("gov.permit.issued"),
            subject: "permit:g12".into(),
            effective_at: 1_700_000_050,
            payload_ref: HashRef::new(HashAlgorithm::Sha256, vec![0x82u8; 32]).unwrap(),
    ...
    let evd = make_evidence(
        ...
        HashRef::new(HashAlgorithm::Sha256, vec![0x83u8; 32]).unwrap(),
        Some(att.id.clone()),
        None,
        &lim,
    )
