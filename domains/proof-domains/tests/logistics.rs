//! Logistics domain journey (shipment vertical). Domain DATA only.
...
pub fn journey() -> BuiltProof {
    ...
    let e1 = create_event(
        proof_core::model::EventContent {
            v: 1,
            event_type: EventType::new("logistics.shipment.dispatched"),
            subject: "shipment:s42".into(),
    ...
    let e2 = create_event(
        proof_core::model::EventContent {
            v: 1,
            event_type: EventType::new("logistics.shipment.delivered"),
            subject: "shipment:s42".into(),
            effective_at: 1_700_000_050,
            payload_ref: HashRef::new(HashAlgorithm::Sha256, vec![0x22u8; 32]).unwrap(),
    ...
    let evd = make_evidence(
        ...
        HashRef::new(HashAlgorithm::Sha256, vec![0x33u8; 32]).unwrap(),
        Some(att.id.clone()),
        None,
        &lim,
    )
    ...
    let mut b = proof_verify::ProofBuilder::new(
        Proposition {
            v: 1,
            kind: "logistics.shipment.delivered".into(),
            subject: e1.id.clone(),
...
pub fn policy() -> serde_json::Value {
    serde_json::json!({
        "policy_version": 1,
        "policy_id": "logistics_delivery_v1",
        "requirements": [
