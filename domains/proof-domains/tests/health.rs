//! Healthcare domain journey (records vertical). Domain DATA only.
...
pub fn journey() -> BuiltProof {
    ...
    let e1 = create_event(
        proof_core::model::EventContent {
            v: 1,
            event_type: EventType::new("health.record.created"),
            subject: "record:r9".into(),
            effective_at: 1_700_000_000,
            payload_ref: HashRef::new(HashAlgorithm::Sha256, vec![0x71u8; 32]).unwrap(),
    ...
    let e2 = create_event(
        proof_core::model::EventContent {
            v: 1,
            event_type: EventType::new("health.consent.granted"),
            subject: "record:r9".into(),
            effective_at: 1_700_000_050,
            payload_ref: HashRef::new(HashAlgorithm::Sha256, vec![0x72u8; 32]).unwrap(),
    ...
    let evd = make_evidence(
        ...
        HashRef::new(HashAlgorithm::Sha256, vec![0x73u8; 32]).unwrap(),
        Some(att.id.clone()),
        None,
        &lim,
    )
    ...
    let mut b = proof_verify::ProofBuilder::new(
        Proposition {
            v: 1,
            kind: "health.consent.granted".into(),
            subject: e1.id.clone(),
