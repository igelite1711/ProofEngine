...
## Operator Answers (read in minutes; spec wins on conflict)

1. **What is Proof Engine?** Neutral, portable proof infrastructure: claims + evidence + attestations + provenance/relationships + cryptographic bindings → verifiable Proof.
2. **What is it not?** Not a blockchain, payment system, identity provider, trust authority, database, SaaS, or AI provenance system.
3. **Why does it exist?** Claims trapped in DBs/APIs need a machine-verifiable answer to “why believe this” that travels without trusting the producer.
4. **What is a proof?** Portable container: proposition + sorted member id sets (events/attestations/evidence/relationships) + `created_at`, bound by `proof_id`.
5. **How do I create one?** Primary path: `create-event → attest → add-evidence → relate → build` (see Quick Start; full map in `docs/CLI.md`).
6. **How do I verify one?** `verify --proof --clock --revocations-known-at [--status --authority | --no-require-status genesis only] [--production]`; `evaluate --policy --trusted` to decide; `explain` to narrate.
7. **What does VALID mean?** Cryptographically + evidentially intact *as recorded* (historically valid). NEVER “safe now” alone.
8. **What does CURRENTLY ACCEPTABLE mean?** Valid AND current (no SUPERSEDED history / unverified hints; revocation fresh). Production MUST use `--production` or check JSON `currently_acceptable`.
9. **What does the engine NOT verify?** Truth of claims, issuer trust (policy does), external bytes (digest-only; core never fetches), wall-clock truth, meaning of open labels.
10. **How can another implementation verify it?** Cryptographic interop core: canonical CBOR + ids + COSE_Sign1 + proof-id binding (`interop/pengine.py`, TS; `make interop`). Full verdicts via `make conformance`; boundary in `INTEROPERABILITY.md`.
11. **How do I integrate it?** `docs/INTEGRATION.md` (Rust one-shot `verify_and_evaluate` / CLI / HTTP) + `docs/OPERATIONS.md` adapter contract (registries, rotation, feeds, replay, clocks) + `docs/OPERATOR-RUNBOOK.md` checklist.
12. **What is frozen?** Proof/claim/evidence/attestation/relationship/identity-binding semantics, canonicalization, proof identity, verification/lifecycle/crypto-abstraction/version semantics (`ARCHITECTURE-FREEZE.md`, `make freeze-guard`).
13. **What is extensible?** Algorithms, adapters, domain profiles, policy versions (v1 frozen, v2 additive; see `POLICY.md` migration), external stores, transparency/identity/transports, CLI conveniences, SDKs.
...
# UNKNOWN and verify fails closed (exit 1) with a hint explaining production
# vs historical paths (see `proof-cli help verify`).
# GENESIS/BOOTSTRAP ONLY (testing, no revocations can exist yet): assert
# absence explicitly with --no-require-status. NEVER copy into production.
# Production uses --status feed files + --authority keys (no --no-require-status).
...
# Production evaluate/explain (no --no-require-status; feed files required):
# proof-cli evaluate --proof proof.json --policy policy.json --clock 1700000300 \
#     --revocations-known-at 1700000300 --status status.json --authority $ISSUER --trusted $ISSUER --production
proof-cli explain --proof proof.json --policy policy.json --clock 1700000300 \
    --revocations-known-at 1700000300 --no-require-status --trusted $ISSUER

# H1: VALID vs CURRENTLY ACCEPTABLE (distinct, never collapsed).
# --no-require-status PASS means historically valid (JSON currently_acceptable
# may be false on SUPERSEDED/unverified-provenance history). VALID alone is
# NEVER "safe to rely on now". Production MUST use the current-acceptability
# path: --production (implies --require-acyclic + --strict-current + feed gate;
# verify==acceptable) or check `currently_acceptable` in JSON, or `evaluate`
# with not_superseded/evidence_usable/no_conflicting_evidence. History preserved.
# Production verification (feed files required, no --no-require-status):
proof-cli verify --proof proof.json --clock 1700000300 --revocations-known-at 1700000300 --production
# H2: DIGEST PRESENT ≠ content checked. Evidence is digest-bound (proof commits
# to digest/reference); verifier does NOT fetch/check bytes in V1. Operate a
# content store; use opt-in claim evidence_digest + evidence_ref binding where
# values must match bytes (see docs/OPERATOR-RUNBOOK.md §5, docs/INTEGRATION.md).
# H5 interop boundary: independent Python/TS verifiers cover format +
# canonicalization + identifiers + signatures (cryptographic interop core);
# lifecycle/graph/temporal/policy verdicts are engine-side (see INTEROPERABILITY.md).
