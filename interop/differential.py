...
def main():
    ...
    for label, mutate in (
        ...
        # M3 remediation (HARNESS FAILURE, not engine): engine emits the
        # structured code in JSON stdout, human text on stderr. Check stdout.
        rc, out, err = cli(a.proof_cli, "verify", "--proof",
                           os.path.join(a.work, f"unk-{label}.json"),
                           "--clock", "1700000300",
                           "--revocations-known-at", "1700000300")
        check(f"NEG unknown {label} field rejected by Rust",
              rc != 0 and "SCHEMA_VIOLATION" in out, f"exit={rc}")
    ...
    if isinstance(atts, list) and atts:
        ...
        open(os.path.join(a.work, "wrong-ref.json"), "w").write(
            json.dumps({"kind": "proof", "id": "x", "cbor": bad.hex()}))
        # M3 remediation: same stdout-vs-stderr fix as above.
        rc, out, err = cli(a.proof_cli, "verify", "--proof",
