...
Rules (M1 remediation, single interpretation — BUG fix, no semantic change):
`cbor` is authoritative; `id`/`sign1` are re-derived and never trusted.
`kind`, when present on a proof envelope, MUST be exactly `"proof"`:
`verify` rejects any other string (`event`/`evil`/`unknown`/...) with
SCHEMA/SCHEMA_VIOLATION FAIL (exit 1, JSON), never VALID. Missing `kind`
is allowed for raw transports (bare `cbor`+optional `id`; bytes authenticate
themselves). Non-string `kind` is a malformed wrapper (transport error,
exit 2). Every consumer MUST re-derive `id` and, where present, verify
`sign1` from `cbor`. JSON is for human/transport interchange only — never
signed, never canonical. A binary container format (sealed envelope) is a v2 item.
